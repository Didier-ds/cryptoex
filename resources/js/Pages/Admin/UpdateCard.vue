<template>
    <main-layout>
        <div class="mx-auto ">
            ...
            <div class="w-10/12 mx-auto rounded-lg border mt-8 md:p-4 shadow-lg p-2">
            <form @submit.prevent = "submit">
                <div class="md:grid grid-cols-2">
                    <div class="m-2">
                        <p>Please Select your giftcard</p>
                        <select id="giftcard" v-model="form.card" name="giftcard">
                            <option value="" selected>--Please Select your giftcard--</option>
                            <option v-for="card in newGiftcards" :key="card" :value="card">{{card}}</option>
                    ...
                    <div class="m-2">
                        ...
                        <select v-model="form.category" id="category" name="category">
                            <option value="" selected>--Please Select your giftcard category--</option>
                            <option :value="category.type" v-for="category in categories" :key="category">{{category.type}}</option>
                        </select>
                    </div>
                </div>
                <div class="md:grid grid-cols-2 items-center">
                    ...
                    <q-btn color="secondary" :disabled="form.processing" class="my-2 mx-auto" label="Upgrade"  type="submit"/>
            </form>
        </div>
        </div>
    </main-layout>
...
watch(() => form.card, () => {
    categories.value = props.giftcards.filter((card) => card.name === form.card)
    })

const rate = computed(() => {
    const selectedCard = props.giftcards.find(card => card.name === form.card && card.type === form.category)
